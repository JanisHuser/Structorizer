function openStructogramm(fileContext, fileType) {
        ...
        switch(fileType) {
            ...
            for( var i=1; i < split3.length; i++ ) {
                ...
                if( i > 1 || window.location.pathname == "/loadStruct/" || window.location.href.indexOf("structCode") != -1) {
		...
		case "sFile":
			...
			formData.append("code", fileContext);
			$.ajax({
				url: "script/returnsFile.php",
				type: 'post',
				data: formData,
				dataType: 'html',
				async: true,
				...
				success: function(data) {
					var split3 = data.split("mainStructHeaderCompleteLoad:{");
					for( var i=1; i < split3.length; i++ ) {
						$(".structContent").append((split3[i].split("}")[0]));
